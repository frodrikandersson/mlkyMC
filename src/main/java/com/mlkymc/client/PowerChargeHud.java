 ...
 *
 * Only visible for classes that use hold-to-charge (Adventurer Dash).
 * Turns red during overcharge window, then cancels.
 */
public class PowerChargeHud {
    ...
    public void onRenderHud(RenderGuiLayerEvent.Post event) {
        ...
        float overcharge = ModKeybinds.getOverchargePercent(); // 0.0 to 1.0
        ...
        if (overcharge > 0) {
            // Overcharging — bar is red, pulses brighter as it nears cancel
            int pulse = (int) (200 + 55 * overcharge);
            color = 0xFF000000 | (pulse << 16);
        } else if (charge >= 1.0f) {
        ...
        // Border
        g.fill(barX, barY, barX + barWidth, barY + 1, 0xFF333333);
        g.fill(barX, barY + barHeight - 1, barX + barWidth, barY + barHeight, 0xFF333333);
        ...
        g.drawString(mc.font, text, (screenW - textWidth) / 2, barY - 10, textColor, true);
    }
}
